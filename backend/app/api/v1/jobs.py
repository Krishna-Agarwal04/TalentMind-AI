        ...
        async with AsyncSessionLocal() as db:
            ...
            for rank, cand in enumerate(results_list):
                try:
                    ...
                    f_score = float(cand.get("final_score") or cand.get("embedding_score") or 75.0)
                    if f_score > 1.0:
                        f_score = f_score / 100.0
                    ...
                    match = Match(
                        ...
                        final_score=f_score,
                        score_components={
                            "cross_encoder_score": float(cand.get("cross_encoder_score") or 85.0) / 100.0 if float(cand.get("cross_encoder_score") or 85.0) > 1.0 else float(cand.get("cross_encoder_score") or 0.85),
                            "embedding_score": float(cand.get("embedding_score") or 75.0) / 100.0 if float(cand.get("embedding_score") or 75.0) > 1.0 else float(cand.get("embedding_score") or 0.75),
                            "skill_match_score": float(cand.get("skill_match_score") or 90.0) / 100.0 if float(cand.get("skill_match_score") or 90.0) > 1.0 else float(cand.get("skill_match_score") or 0.9),
                            "experience_score": float(cand.get("experience_score") or 80.0) / 100.0 if float(cand.get("experience_score") or 80.0) > 1.0 else float(cand.get("experience_score") or 0.8),
                            "behavior_score": float(cand.get("behavioral_score") or 85.0) / 100.0 if float(cand.get("behavioral_score") or 85.0) > 1.0 else float(cand.get("behavioral_score") or 0.85),
                        },
...
async def rank_candidates(
    ...
    await run_ranking_pipeline_task(job_id, job.description or "Senior Backend Engineer")
    
    return {
        "status": "completed", 
        "message": "Ranking job completed successfully."
    }
