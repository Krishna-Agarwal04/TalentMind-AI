"""Maps recruiter intent to an optimized, bounded tool execution plan."""
...
def create_plan(intent: str, session_state: Optional[Dict[str, Any]] = None) -> List[str]:
    ...
    Uses session context and streamlined pipeline steps for ultra-fast response.
    """
    ...
    if intent == INTENT_RANKING:
        ...
        return ["get_job_context_tool", "rank_candidates_pipeline_tool"]

    if intent == INTENT_EXPLANATION:
        if has_ranked:
            return ["get_job_context_tool", "explain_candidate_tool"]
        return ["get_job_context_tool", "rank_candidates_pipeline_tool", "explain_candidate_tool"]
